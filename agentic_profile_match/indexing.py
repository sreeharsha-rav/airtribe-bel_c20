"""Resume indexing: batch metadata extraction, section-aware chunking, and
hybrid dense+sparse embedding/upsert into this project's own Qdrant instance.

Ported from rag_profile_match/resume_rag.py (that project's indexing side)
now that agentic_profile_match owns its own Qdrant instance and its own
data/ corpus, rather than reading rag_profile_match's. Function libraries
only -- no top-level execution; see scripts/reindex.py for the actual
pipeline run. Embedding-model builders are NOT duplicated here -- they
already live in retrieval.py (shared with query-time embedding) and are
imported from there.
"""
...
class ResumeFields(BaseModel):
    ...
    skills: list[str] = Field(
        ...
        description=(
            ...
            "Lowercase, deduplicated, canonical names (e.g. 'python', 'aws', 'sql')."
        ),
    )
...
def infer_dept(relative_path: str) -> str:
    """Infers the department from a resume's path relative to data/.

    list_files() (fs_tools.py) reports paths relative to ROOT_DIR, so a
    resume under resumes/<dept>/... arrives as ("resumes", "<dept>", ...). A
    file sitting directly under resumes/ (no subfolder) belongs to no
    department.
    """
...
def build_extraction_model() -> Runnable[LanguageModelInput, ResumeFields]:
    ...
    ResumeFields class (not include_raw=True) always yields a ResumeFields instance
    at runtime, so the cast narrows that back to the real type.
    """
...
def discover_resume_files() -> list[dict]:
    """Lists every resume file under data/resumes via the sandboxed fs tool."""
...
def extract_fields_batch(
    ...
    one concurrent `.batch()` call per chunk. return_exceptions=True isolates
    one resume's extraction failure to that resume's slot instead of
...
def embed_chunks(
    ...
    Batch-embeds once per model rather than one call per chunk. Returns
    chunks with "dense_embedding" (list[float]) and "sparse_embedding"
    ({"indices": list[int], "values": list[float]}) added, ready for
    upsert_chunks.
    """
...
def ensure_qdrant_collection(vector_size: int) -> None:
    ...
    if not config.client.collection_exists(config.QDRANT_COLLECTION_NAME):
        config.client.create_collection(
            ...
            vectors_config={
                "dense": qdrant_models.VectorParams(size=vector_size, distance=qdrant_models.Distance.COSINE),
            },
            sparse_vectors_config={
                "sparse": qdrant_models.SparseVectorParams(modifier=qdrant_models.Modifier.IDF),
            },
        )
